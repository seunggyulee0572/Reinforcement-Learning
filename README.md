### 💡 **`music_300.pkl` 파일 경로 수정 안내**

모델을 실행하고 테스트하기 위해서는 **'멜로디 환경 구성'** 섹션부터 순서대로 진행하시면 됩니다.
특히, 아래 코드 셀에 표시된 부분처럼 `data.pkl` 파일을 불러오는 코드의 경로를 제공받으신 `music_300.pkl` 파일의 실제 경로로 수정해야 합니다. 이 파일은 전처리된 멜로디 데이터가 포함되어 있습니다.
